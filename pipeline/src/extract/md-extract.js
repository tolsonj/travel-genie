...
import {
  resolveRoutePreset,
  parseMapNodesFromMarkdown,
  resolveProfileSeason,
  parseCountriesLabel,
  buildRouteName,
  parseRouteLabel,
  parseDepartNode
} from "../geo/preset-resolver.js";
import { REPO_ROOT } from "../discover.js";

function readOptionalProfile(trip) {
  for (const file of ["profile.md", "opt-01-traveler-profile.md"]) {
    const p = join(REPO_ROOT, "trips", trip, file);
...
function extractRouteOptimization(md, meta) {
  ...
  const preset = resolveRoutePreset(meta.trip, md, { refresh: !!meta.refreshGeo });
  ...
  const hubs = (hubTable?.rows || []).map(row => {
    ...
    else if (/bangkok/i.test(hub)) hub = "Bangkok";
    else if (/chiang mai/i.test(hub)) hub = "Chiang Mai";
    else if (/krabi|ao nang|kbv/i.test(hub)) hub = "Krabi";
    else if (/koh samui|samui/i.test(hub)) hub = "Koh Samui";
  ...
  const mdNodes = parseMapNodesFromMarkdown(body);
  const hubCoords = preset.hubCoords || {};
  const map_nodes = mdNodes.length
    ? mdNodes
    : uniqHubs
        .filter(h => !/^home$/i.test(h.hub))
        .map(h => {
          const key = Object.keys(hubCoords).find(
            k => h.hub.includes(k) || k.includes(h.hub.split("/")[0].trim())
          );
  ...
  const profileMd = readOptionalProfile(meta.trip);
  const routeName = uniqHubs.length ? buildRouteName(uniqHubs) : preset.name;
  const routeLabel = uniqHubs.length
    ? parseRouteLabel(profileMd, uniqHubs)
    : (preset.route_label || "Recommended Route (14 Days)");
  const departNode = uniqHubs.length
    ? parseDepartNode(body, uniqHubs)
    : preset.depart_node;
  ...
  return {
    ...
    map_bounds: mdNodes.length
      ? {
          west: Math.min(...mdNodes.map(n => n.lng)) - 2,
          east: Math.max(...mdNodes.map(n => n.lng)) + 2,
          south: Math.min(...mdNodes.map(n => n.lat)) - 2,
          north: Math.max(...mdNodes.map(n => n.lat)) + 2
        }
      : preset.map_bounds,
    highlight_countries: preset.highlight_countries,
    country_labels: preset.country_labels,
    depart_node: departNode,
    transit_legs,
...
function extractTravelerProfile(md, meta) {
  ...
  const core =
    findTable(tables, /traveler summary|core ident/i) ||
    tables.find(
      t =>
        t.columns.some(c => /attribute/i.test(c)) && t.columns.some(c => /detail/i.test(c))
    );
  const budgetTbl = findTable(tables, /^budget$/i) || findTable(tables, /splurge allocation/i);

  const countries = fieldFromTable(core, /countries/i);
  const window = fieldFromTable(core, /travel window|dates/i);
  const party = fieldFromTable(core, /party|companions|passengers/i);
  const totalBudget = fieldFromTable(budgetTbl, /^total$/i) || fieldFromTable(core, /budget/i);
  const homeAirport =
  ...
  return {
    ...
    metrics,
    seasonality: resolveProfileSeason(meta.trip, md),
